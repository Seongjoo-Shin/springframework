...
public class Ch07Controller {
	...
	public String requestScopeSave(HttpServletRequest request) {
		// request 범위에 객체 저장
		log.info("실행");
		request.setAttribute("requestScopeValue", "홍길동1");
		
		// 멤버 객체 생성
		// 디비에서 불러온 데이터를 보통 이렇게 저장해서 뷰로 넘겨줌
	...
	public String sessionScopeSave(HttpSession session) {
		// session 범위에 객체 저장
		log.info("실행");
		session.setAttribute("sessionScopeValue", "홍길동2");
		
		// 멤버 객체 생성후 session 범위 객체에 저장
		Ch07Member member = new Ch07Member();
		member.setName("감자바");
		member.setAge(27);
		member.setJob("개발자");
	...
	public String applicationScopeSave(HttpServletRequest request) {
		...
		// application을 얻기위해 Servletcontext 사용
		ServletContext application = request.getServletContext();
		application.setAttribute("applicationScopeValue", "홍길동3");
		
		// 객체 생성후 application 범위에 저장
	...
	public String useJstl1(HttpServletRequest request) {
		String[] languages = { "Java", "JavaScript", "SpringFramework", "VueJS" };
